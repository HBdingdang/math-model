...
A=O_0+r*n1_0;
B=O_0-r/2*n1_0+sqrt(3)/2*r*n2_0;            %A,B,C坐标
...
for i=1:n-1
    A=O(i,:)+r*n1(i,:);
    B=O(i,:)-r/2*n1(i,:)+sqrt(3)/2*r*n2(i,:);            %A,B,C坐标
...
[x,y]=meshgrid(a,b);
for i=1:100:n
    X=x.*cos(y)*n1(i,1)+x.*sin(y)*n2(i,1)-sqrt(1-x.^2)*n3(i,1)+O(i,1);
    Y=x.*cos(y)*n1(i,2)+x.*sin(y)*n2(i,2)-sqrt(1-x.^2)*n3(i,2)+O(i,2);
    Z=x.*cos(y)*n1(i,3)+x.*sin(y)*n2(i,3)-sqrt(1-x.^2)*n3(i,3)+O(i,3);
	mesh(X,Y,Z);
    pause(0.1);
end
...
y(4:6)=cross(l1,F1)+cross(l2,F2)+cross(l3,F3)-F*cross(l1,[0,0,1]);  % 合力矩大小
end
